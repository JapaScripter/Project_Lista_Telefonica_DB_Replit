...
5 - para ver todos os contatos
6 - para deletar todos os contatos
----------------------------------
"""


def prompt_delete_all_contatos():
	contatos.delete_all_contatos()
...
def prompt_add_contato():
	...
	telefone = limit_phone_number(input("Coloque o número de telefone: ").strip())
	print(
	 f"O nome: {nome} com o telefone: {telefone} foi adicionado com sucesso!")
...
def print_all_contatos():
	for key in contatos.db.keys():
		print(f"Name: {key} - Phone: {contatos.db[key]}")
...
def prompt_atualizar_contato():
	...
	if velho_telefone:
		novo_nome = input(
		 f"Para qual nome você deseja mudar o {velho_nome} (deixe em branco se nãp quiser mudar): "
		).strip()
		novo_telefone = input(
		 f"Para qual telefone você deseja mudar o {velho_telefone} (deixe em branco se nãp quiser mudar): "
		).strip()
